...
@Controller
@RequestMapping("product")  // валидация цены и мануфактурера у продукта + длины
public class ProductController {
    ...
    public ModelAndView updatePost(@NotNull Product product, ModelMap model) {
        if (product.getId() == null) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Product ID Is Null").addAttribute("error2", "Please, Try Again"));
        }
        if (product.getName() == null) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Product Name Is Null").addAttribute("error2", "Please, Try Again"));
        }
        if (product.getPrice() == null) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Product Price Is Null").addAttribute("error2", "Please, Try Again"));
        }
        if (product.getManufacturer() == null) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Product`s Manufacturer Is Null").addAttribute("error2", "Please, Try Again"));
        }
        if (product.getId().isEmpty()) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Product ID Is Empty").addAttribute("error2", "Please, Try Again"));
        }
        if (product.getName().isEmpty()) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Product Name Is Empty").addAttribute("error2", "Please, Try Again"));
        }
        if (!Validator.validId(product.getId())) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Wrong ID").addAttribute("error2", "Please, Try Again"));
        }
        if (!Validator.validName(product.getName())) {
            return new ModelAndView("product/updateProduct", model.addAttribute("product", product).addAttribute("error", "Wrong Name").addAttribute("error2", "Please, Try Again"));
        }
        product.setName(product.getName().toUpperCase());
        productService.saveEntity(product);
        return new ModelAndView("product/product", model.addAttribute("product", product).addAttribute("error", " Product Updated").addAttribute("error2", "SUCCESSFULLY"));
    }
}