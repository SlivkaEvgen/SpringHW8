...
public class RoleController {
    ...
    public ModelAndView delete(String id, ModelMap model) {
        ...
        if (!Objects.equals(model.getAttribute("role"), "ADMIN")) {
    ...
    public ModelAndView addNewPost(@NotNull Role role, ModelMap model) {
        ...
        if (roleService.getById(role.getId()).isPresent()) {
            return new ModelAndView("role/newRole", model.addAttribute("role", role).addAttribute("error", "Role With ID " + role.getName() + " Is Used").addAttribute("error2", "Please,Try Again"));
        }
    ...
    public ModelAndView updatePost(@NotNull Role role, ModelMap model) {
        ...
        if (!Validator.validId(role.getId())) {
            return new ModelAndView("role/updateRole", model.addAttribute("role", role).addAttribute("error", "Wrong Role ID").addAttribute("error2", "Try Again"));
        }
        if (!Validator.validName(role.getName())) {
            return new ModelAndView("role/updateRole", model.addAttribute("role", role).addAttribute("error", "Wrong Role Name").addAttribute("error2", "Try Again"));
        }
        if (!roleService.getById(role.getId()).isPresent()) {
